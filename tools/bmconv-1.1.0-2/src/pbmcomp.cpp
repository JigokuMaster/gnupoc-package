	...
	while(bytepointer<limitmargin)
		...
		if(*(bytepointer+1)==value && *(bytepointer+2)==value)
			...
			while(*bytepointer==value && bytepointer<limitmargin)
				bytepointer++;
			ret = WriteCompressedByteValues(aDest,value,bytepointer-oldbytepointer);
			if(ret) return ret;
			}
		else
			{
			...
			ret = WriteCompressedByteData(aDest,oldbytepointer,bytepointer-oldbytepointer);
			if(ret) return ret;
			}
...
int BitmapCompiler::CompressSixteenBitData(char*& aDest,char* aSrce,int aSizeInBytes)
	...
	while (srcePtr < srceLimitPtrMinusOne)
		...
		if(*srcePtr == value)
			...
			ret = WriteCompressedSixteenBitValues(aDest,value,srcePtr-runStartPtr);
			if (ret)
				return ret;
			}
		else
			...
			ret = WriteCompressedSixteenBitData(aDest,runStartPtr,srcePtr - runStartPtr);
			if (ret)
				return ret;
			}
	...
	if (srcePtr < srceLimitPtr)
		{
		ret = WriteCompressedSixteenBitData(aDest,srcePtr,srceLimitPtr - srcePtr);
		if (ret)
			return ret;
		}
...
int BitmapCompiler::CompressTwentyFourBitData(char*& aDest,char* aSrce,int aSizeInBytes)
	...
	while (aSrce < srceLimitPtrMinusThree)
		...
		if (TrueColorPointerCompare(aSrce,component1,component2,component3))
			...
			ret = WriteCompressedTwentyFourBitValues(aDest,component1,component2,component3,(aSrce - runStartPtr) / 3);
			if (ret)
				return ret;
			}
		else
			...
			ret = WriteCompressedTwentyFourBitData(aDest,runStartPtr,(aSrce - runStartPtr) / 3);
			if (ret)
				return ret;
			}
		if (aDest > destCompressedLimitPtr)
	...
	if (aSrce < srceLimitPtr)
		{
		ret = WriteCompressedTwentyFourBitData(aDest,aSrce,(srceLimitPtr - aSrce) / 3);
		if (ret)
			return ret;
		}
